...
const { line, cube, rectangle, sphere, polygon } = jscad.primitives
...
function getParameterDefinitions(){
  return [
    {name:'firstTriangle', type:'number', caption:'Start Face',initial:0,step:1},
    {name:'Pscale', type:'number', caption:'Scale',initial:80, default:80},
    {name:'frameX', type:'number', caption:'Frame X',initial:210, default:210},
    {name:'frameY', type:'number', caption:'Frame Y',initial:297, default:297}
    ,{name:'fileN', type:'text', caption:'File', initial:'change.obj'}
  ]
} 

const main = (params) => {
  // INITS
  cachedNum = [], cachedDig = [], cacheKO = [], lUNFOLD = []
  g_s = params.Pscale, gSCALE = 0.1
  frame = [params.frameX, params.frameY]
  const vf = require('./'+ params.fileN)
  vol = toPolyhedron(union(vf))
  //vol = toPolyhedron(sphere({segments:12}))
...
function eq4(v1, v2){
	return distance2d(v1, v2) < epsilon
}

function doLine(p1, p2, n, nf){
  var l = lLINES.find(v => (eq4(v[0], p1) && eq4(v[1], p2))
												|| (eq4(v[0], p2) && eq4(v[1], p1)))
  if( l === undefined){
