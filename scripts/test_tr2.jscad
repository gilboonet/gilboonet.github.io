...
function getParameterDefinitions() {
  return [
    ...
    { name: 'mode',type: 'choice', initial: 1, caption:'Mode', captions:['Vue 3d','Vue 3d eclatee', 'Vue 2d'], values:[30,31,2]},
...
for(i = 0.5; i< params.nbL; i++){ // lignes
  tr =rotate([90,0,0], faitLigne(i, v, b, d, params.nbL, params.eC));
  bz = tr.getBounds()[0].z;
...
for(i = 0.5; i < params.nbC; i++){ // colonnes
  tr =rotate([0,90,0], faitColonne(i, v, b, d, params.nbC, params.eC));
  bz = tr.getBounds()[0].z;
  tmp = vol2surf(tr, bz).concat();
  for(j = 0; j < tmp.length; j++){
    tr = tmp[j];
    tr= translate([-bz, 0, 0], rotate([0,-90,0], tr.extrude({offset:[0,0,params.eC]})));
    rC = rC.concat(tr);
  }
}
urC = union(rC);
...
function tourneEt2d(vol, angle){
  ...
  return union(vol2surf(t, t.getBounds()[0].z));
}
